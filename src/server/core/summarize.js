...
function fullSummary(row, options, index) {
  const overview = EventData.getSummary(row, options);
  const summary = DailyData.getSummary(row);
  const endtime = dt.to24HourString(EventData.getEndTime(row));
...
const summarize = {
  ...
  count(options) {
    // forces unordered regardless of caller's desire.
    const combined = getDefaultOptions(options, {newestFirst: 'unordered'});
