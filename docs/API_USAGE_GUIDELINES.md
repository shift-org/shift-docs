...
1. Do not abuse our service with queries (time range or single event queries should each be made a maximum of once a day and at a rate of less than one per second.  requests should be made sequentially, not in parallel)
...
7. All event data is presumed to be the property of the ride creator.
...
3.  please do not scrape full HTML pages from our website; all data is available via API
