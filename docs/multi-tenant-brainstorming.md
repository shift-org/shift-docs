...
call for the calendar page via id/category in calendar.md is routed to:
	/Users/fool/Desktop/code/shift-docs/site/themes/s2b_hugo_theme/layouts/partials/cal/fullcal.html

from Simon:

this is the plugin we use for displaying the calendar : https://fullcalendar.io/
...
### Goals to meet for  actually having anyone use this:
...
- separate DB tables for their events means we could scope mysql access to keep them from accidentally editing our events
- separate docker containers is also pretty cheap and allows separate restarts even on just the one machine
-  
