...
export default {
  ...
  async getDaily(caldaily_id, options = null) {
    ...
    if (cached) {
      ...
      // grab one event:
      const url = calApi.Events.Build({id: caldaily_id});
  ...
  async getRange(start, end) {
    ...
    if (_lastRange.key === key) {
      ...
      const url = calApi.Events.Build({ startdate, enddate });
  ...
  async getSearch(searchStr, offset, searchAll) {
    const url = calApi.Search.Build({ q: searchStr, o: offset, all: searchAll });
    console.log(`fetching ${url}`);
