...
Or, in the test code, they can be temporarily marked with '.only()'. For example: `describe.only("ical feed")`, and then isolated with: `npm test -- -only`. ( '.skip()' is another option to ignore certain tests temporarily. )
