        ...
        var opts = {
            ...
            success: function(data) {
                ...
                $.each(data.events, function( index, value ) {
                  ...
                  let calDates = dayjs(`${value.date} ${value.time}`).toISOString()
                  if (value.eventduration) {
                    const endDate = dayjs(calDates).add(dayjs.duration({ 'minute': value.eventduration })).toISOString()
                    calDates = `${calDates.replace(/[^\w\s]/gi, '')}/${endDate.replace(/[^\w\s]/gi, '')}`
                  } else {
                    calDates = calDates.replace(/[^\w\s]/gi, '')
                  }
