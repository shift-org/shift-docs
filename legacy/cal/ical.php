...
METHOD:PUBLISH
PRODID:-//Shift Calendar//Bike Fun//EN
X-WR-CALNAME;VALUE=TEXT:Bike Fun
VERSION:2.0
<?php
    # For each event...
    $sequence = 0;
    ...
    $lastdate = date('Y-m-d', time() + 86400 * 7 * $numweeks);
    $result = mysql_query("SELECT * FROM calevent, caldaily WHERE eventdate >= '$firstdate' AND eventdate <= '$lastdate' AND eventstatus <> 'C' AND eventstatus <> 'E' AND eventstatus <> 'S' AND review <> 'E' AND calevent.id = caldaily.id ORDER BY eventdate, eventtime", $conn) or die(mysql_error());
	...
	print "ATTACH:".CALURL."view3week.php#$mday-$id\n";
	print "END:VEVENT\n";
    }
    #ex:set sw=4 it=s:
?>
