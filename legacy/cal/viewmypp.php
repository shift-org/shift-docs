<?php
# This is a simple list of the *palooza events.  Users can select events
...
#               the date, relative to PPSTART.  The rest is a base-36 encoding
#               of the event ID.
#
#   sharename   Name of a shared list to update
	...
	if ($_REQUEST["myevents"]) {
	    $sql = "UPDATE calshare SET shareevents='".$_REQUEST["myevents"]."' WHERE sharename='".addslashes($_REQUEST["sharename"])."'";
	    $result = mysql_query($sql, $conn) or die("Updating:".mysql_error());
	    if (mysql_affected_rows($conn) < 1) {
		$sql = "INSERT INTO calshare (shareevents, sharename) VALUES ('".$_REQUEST["myevents"]."', '".addslashes($_REQUEST["sharename"])."')";
		$result = mysql_query($sql, $conn) or die("Adding to calshare: ".mysql_error());
	    }
	} else {
	    $sql = "DELETE FROM calshare WHERE sharename='".addslashes($_REQUEST["sharename"])."'";
...
/* Generate a string that uniquely identifies a *palooza event, given its
 * date and id.
 */
...
function initcheckboxes(list)
{
    ...
    /* Enable the [Recall] button iff there's a "storedevents" cookie */
    var stored = readCookie("storedevents");
    setClass(document.getElementById("recall"), (!stored || stored == "") ? "disabled" : "button");
}

/* Uncheck all checkboxes, and delete the cookie */
function resetcheckboxes(forced)
{
    var i, id, sqldate;
    var inputs = document.getElementsByTagName("input");

    if (!forced && !confirm("Are you sure you want to uncheck all event boxes?"))
	return;
	...
	if (inputs[i].type == "checkbox"
	 ...
	 && inputs[i].id.indexOf("-") >= 0) {
	    ...
	    /* reset the row color */
	    tr = document.getElementById("row"+inputs[i].id);
	    tr.style.backgroundColor = ""; //"#ffe080";
	}
    }
...
function useshare(sharename, shareevents)
{
    ...
    if (document.getElementById("count").innerhtml + 0 > 0 && anyadded) {
...
function storelist()
{
    // Fetch the current event list
    var myevents = readCookie("myevents");
    if (!myevents || myevents == "") {
	alert("You can't store an empty event list");
	return;
    }

    // Copy the list from the "myevents" to "storedevents"
    createCookie("storedevents", myevents);

    // Enable the [Recall] button
    setClass(document.getElementById("recall"), "button");

    // Clobber the "myevents" list
...
function recalllist()
{
    var stored = readCookie("storedevents");
    if (!stored || stored == "") {
	//You don't have any stored events
	return;
    }
    ...
    // Add the events from the stored list
    var inputs = document.getElementsByTagName("input");
    for (i = 0; i < inputs.length; i++) {
	if (inputs[i].type == "checkbox"        /* it's a checkbox */
	 && inputs[i].id                        /* it has id, not just name */
	    ...
	    // If in the list, then turn it on
	    if (ischecked(sqldate, id, stored) && !inputs[i].checked) {
		inputs[i].checked = true;
		toggle(true, sqldate, id);
	    }
	}
...
<style type="text/css">
  td.date {font-weight: bold; font-size: larger; background: url(images/oocorner.gif) no-repeat;}
    ...
    a.button { border: medium outset #ffd080;
	       ...
	       cursor: pointer;
	       white-space: nowrap;}
    a.disabled { border: medium outset #ffd080;
	       background: #ffd080;
	       color: gray;
	       text-decoration: none;
	       padding-left: 2px; padding-right: 2px;
...
<div class=content style="margin: 5px;">
  <center>
    <h1>Custom <?php print PPNAME; ?> Event List</h1>
    <a class=button style="font-size: x-small;" href="<?php print PPURL; ?>">Return to the <?php print PPNAME; ?> Calendar</a>
  </center>
  <p>
  This is a simple listing of the <?php print PPNAME; ?> events.
  ...
  <a class=button href="#" onClick="resetcheckboxes(false); return false;">Clear My List</a>.
  <p>
<?php
	...
	while ($record = mysql_fetch_array($result)) {
	    print "<a href=\"#\" class=button onClick=\"useshare('".addslashes($record[sharename])."', '".$record[shareevents]."'); return false;\">".htmlspecialchars($record[sharename])."</a>\n";
	}
	print "  </div>";
	print "  <p>\n";
    }
...
<?php
    # For each event...
    $result = mysql_query("SELECT * FROM calevent, caldaily WHERE eventdate >= '".PPSTART."' AND eventdate <= '".PPEND."' AND eventstatus <> 'C' AND eventstatus <> 'E' AND eventstatus <> 'S' AND calevent.id = caldaily.id ORDER BY eventdate, eventtime", $conn) or die(mysql_error());
    $prevdate = "";
    while ($record = mysql_fetch_array($result)) {
	...
	print "        <a href=\"".PPURL."#$htmlid\" target=\"pp\"$titlecolor title=\"".htmlspecialchars($record["title"])."\">\n";
...
?>
  ...
  <div class="form">
    <a class=button href="viewmyfull.php">Full descriptions</a> exactly like the main calendar view
  </div>
  ...
  <div class="form">
    <form action="viewmypp.php" method="POST" onSubmit="return confirm('Do you really want to share this list with the public?')">
  ...
  <h2>Store/Recall Your List</h2>
  You can only be editing one list list at a time.
  If you've set up your personal list, and then decide you'd also like to
  create or extend a shared theme list, the way to do it is to store your
  personal list, then create/edit/share the shared list, and then recall your
  personal list.
  <div class="form">
    <a class=button onClick="storelist()">Store Personal List</a>
    <a class=disabled onClick="recalllist()" id="recall">Recall Personal List</a>
  </div>
</div>
<script type="text/javascript">
initcheckboxes();
guesspocketmod();
document.title = "Custom <?php print PPNAME; ?> Event List";
</script>
<?php

    include(INCLUDES."/footer.html");
    #ex:set sw=4 it=s embedlimit=99999:
?>
