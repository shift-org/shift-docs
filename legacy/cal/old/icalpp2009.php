...
<?php
    # For each event...
    $sequence = 0;
    $result = mysql_query("SELECT * FROM calevent, caldaily WHERE eventdate >= '$startdate' AND eventdate <= '$enddate' AND eventstatus <> 'C' AND eventstatus <> 'E' AND eventstatus <> 'S' AND review <> 'E' AND calevent.id = caldaily.id ORDER BY eventdate, eventtime", $conn) or die(mysql_error());
	...
	print "ATTACH:".CALURL."viewpp2009.php#$mday-$id\n";
	print "END:VEVENT\n";
    }
    #ex:set sw=4 it=s:
?>
